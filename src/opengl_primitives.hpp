#ifndef GATHERING_OPENGL_PRIMITIVES
#define GATHERING_OPENGL_PRIMITIVES
...
struct GLBuffer {
    ...
    GLBuffer(const GLenum usage = GL_STATIC_DRAW) : usage(usage) {}
...
struct Object {
    ...
    /**
     * @brief Adds the vertices and elements of the given object to this object. All other
     * properties (e.g. VAO, name) of the given object are ignored. Elements are added with an
     * offset corresponding to the current number of vertices. So ensure that the data type for
     * the elements offers enough space!
     */
...
struct ObjectInfo {
    ...
    ObjectInfo(size_t vert, size_t elem, GLenum draw_mode)
        : number_vertices(vert), number_elements(elem), gl_draw_mode(draw_mode) {}
    ...
    /**
     * @brief Objects can be drawn with different glsl-programs and different VAO's. In order
     * to draw an object, we have to tell GL which VAO/program shall be used. In case this
     * VAO/program is different to currently used one, GL has to perform state changes. We want
     * to reduce these state changes by drawing all objects with the same VAO/program
     * successively. This is why we sort the Objects by their used VAO/program. We also prefer
     * reduced program swaps over VAO.
     */
...
/**
 * @brief Creates list of vertices that form a cube.
 */
Object createCube(const float size,
                  const glm::vec3 center = glm::vec3(0.f),
                  const glm::vec4 color = glm::vec4(1.f));

/**
 * @brief Creates list of vertices that form a rectangle.
 */
Object createCuboid(const glm::vec3 size,
                    const glm::vec3 center = glm::vec3(0.f),
                    const glm::vec4 color = glm::vec4(1.f));
...
/**
 * @brief Creates a list of elements that form a pipe like shape (around y-axis). Centered at
 * (0,0,0).
 */
Object createPipe(const float radius,
                  const float height,
                  const glm::vec4 color,
                  const unsigned int sector_count = 15u);

/**
 * @brief Creates a list of elements that form a cone (around y-axis). Centered at (0,0,0); top
 * points to (0, h/2, 0).
 */
